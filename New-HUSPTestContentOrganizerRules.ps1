...
function New-HUSPContentOrganizerRules {
    ...
    $SPLib = $SPWeb.Lists[$lib]
    
    # Set content type we are using
    $SPContentType = $SPLib.ContentTypes | Where-Object { $_.Name -ne "Folder" }
    ...
    $term =  $tset.GetTerms($SPLib.Title,$true) | Where { $_.Parent.Name -eq $SPContentType.Name }
    $termValueGuid = $term.Id
    # $termValueGuid        
    
    # Create valid term label
    $committeeName = $SPLib.Title
    $committeeField = [Microsoft.SharePoint.Taxonomy.TaxonomyField]$SPLib.Fields[$SPContentType.Name]
